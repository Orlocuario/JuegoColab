...
public class DisplayHUD: MonoBehaviour{
    ...
    public void CurrentHP(string message)
    {
        ...
        float hpCurrentPercentageFloat = float.Parse(hpCurrentPercentage);


        if (hpCurrentPercentageFloat <= 0.2f)
        {
            percentageText.text = "<color=#e67f84ff>" + (hpCurrentPercentageFloat * 100).ToString("0") + "%" + "</color>";
        ...
        else if (hpCurrentPercentageFloat <= 0.5f)
        {
            percentageText.text = "<color=#f9ca45ff>" + (hpCurrentPercentageFloat * 100).ToString("0") + "%" + "</color>";
        ...
        {
            percentageText.text = "<color=#64b78eff>" + (hpCurrentPercentageFloat * 100).ToString("0") + "%" + "</color>";
        ...
        float currentX = hpCurrentPercentageFloat * maxLimitWidth;      
    ...
    public void CurrentMP(string message)
    {
        ...
        float mpCurrentPercentageFloat = float.Parse(mpCurrentPercentage);

        percentageText.text = (mpCurrentPercentageFloat * 100).ToString("0") + "%";
        ...
        float currentX = mpCurrentPercentageFloat * maxLimitWidth;
    ...
    public void ExperienceBar(string message)
    {
        ...
        float expCurrentPercentageFloat = float.Parse(expCurrentPercentage);

        Text percentageText = GameObject.Find("ExpPercentage").GetComponent<Text>();
        percentageText.text = "Exp: " + (expCurrentPercentageFloat * 100).ToString("0") + "%";
        ...
        float currentX = expCurrentPercentageFloat * maxLimitWidth;
